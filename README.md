...
- `web/`: TanStack Start landing page, shareable live viewer, broadcast studio, and OBS map/metric browser sources
...
The normal Compose stack never builds the API. It uses the image selected by
`RUNSYNC_API_IMAGE`, defaulting to the known-good server revision
`runsync-api:8e74040a2676`. This keeps frontend-only `docker compose up --build`
iterations from replacing the running server with source from an older
workspace. Building and advancing the API pin is an explicit deployment step
described in `docs/server-operations.md`.
